...
def generate_script(state: PipelineState) -> PipelineState:
    ...
    system_prompt = (
        ...
        "6. **Premium Slide Layout**: To prevent faces/subjects from being cut off in vertical/portrait/cropped photos, design the Slide component to overlay two image tags: a background blurred image (styled with `position: 'absolute', width: '100%', height: '100%', objectFit: 'cover', filter: 'blur(20px) brightness(0.4)'`) and a foreground clean image (styled with `position: 'relative', height: '100%', maxWidth: '100%', objectFit: 'contain'`). The parent container wrapping these elements MUST center the foreground image horizontally and vertically using Flexbox: `{ display: 'flex', justifyContent: 'center', alignItems: 'center', position: 'relative', width: '100%', height: '100%', overflow: 'hidden' }`. **Caption Readability & Styling**: To make captions highly readable and ensure they don't overlap with media seekbars:\n"
        "   - Position the caption at least 80px from the bottom: `{ position: 'absolute', bottom: '80px', left: 0, right: 0, textAlign: 'center', color: 'white', fontSize: '32px', fontFamily: 'sans-serif', textShadow: '0px 2px 8px rgba(0,0,0,0.8)', padding: '0 40px', zIndex: 10 }`.\n"
        "   - Apply a subtle dark gradient overlay at the bottom using the `background` property (do NOT use `backgroundColor` for linear gradients): `{ position: 'absolute', bottom: 0, left: 0, right: 0, height: '30%', background: 'linear-gradient(to top, rgba(0,0,0,0.8), transparent)', pointerEvents: 'none' }`.\n"
